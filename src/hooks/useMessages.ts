...
export function useMessages(conversationId: string | null) {
  ...
  const conversationMessages = useChatStore((s) => conversationId ? s.messages[conversationId] : undefined)
  ...
  const stableRef = useRef({ userId, getOrDeriveSharedKey })
  stableRef.current = { userId, getOrDeriveSharedKey }
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      const { userId } = stableRef.current
      if (!userId) return
      try {
        ...
        useChatStore.getState().setMessages(conversationId, decrypted)
      } catch (err) {
      ...
      .on(
        ...
        async (payload) => {
          try {
            ...
            if (dm) {
              useChatStore.getState().addMessage(conversationId, dm)
  ...
  return { messages: conversationMessages ?? EMPTY_MESSAGES }
}
